...
**[Download the latest APK and release files](https://github.com/alferiko/half-life-2-stanalone-quest-releases/releases/latest)**

**Supported content:** Half-Life 2 · Lost Coast · Episode One · Episode Two · Portal 1 | **Headsets:** Meta Quest 3 · Meta Quest 3S

An unofficial, free and non-commercial standalone VR port for Meta Quest 3 and Quest 3S. Version 0.988 supports Half-Life 2, Lost Coast, Episode One, Episode Two and Portal 1 in one Android package. The original games are not included: you must own and install them legally on Steam.

The repository includes the Windows and Linux cache builders in `BUILD_GAME_CACHE.bat`, `BUILD_GAME_CACHE.sh` and `tools/`, plus the complete VR compatibility overlay in `vr_game_resources/`. Download `HL2VR-Standalone-0.988.apk` or the complete archive from the 0.988 release attachments.
...
- the 0.988 APK from the release attachments; either clone this repository or download the complete distribution archive.

## Installation

### 1. Build the legal game cache

1. Clone/download this repository to a writable folder, or extract `HL2VR-Standalone-0.988.7z`. Keep `BUILD_GAME_CACHE.*`, `tools/` and `vr_game_resources/` beside one another.
...
```bash
adb devices
adb install -r HL2VR-Standalone-0.988.apk
```
