...
An unofficial, free and non-commercial standalone VR port for Meta Quest 3 and Quest 3S. Version 0.986 supports Half-Life 2, Lost Coast, Episode One, Episode Two and Portal 1 in one Android package. The original games are not included: you must own and install them legally on Steam.

The repository includes the Windows and Linux cache builders in `BUILD_GAME_CACHE.bat`, `BUILD_GAME_CACHE.sh` and `tools/`, plus the complete VR compatibility overlay in `vr_game_resources/`. A clone is therefore sufficient to build the shared cache. Download `HL2VR-Standalone-0.986.apk` or the complete archive from the 0.986 release attachments.
...
- the 0.986 APK from the repository root or release attachments; either clone this repository or download the standalone distribution archive.

## Installation

### 1. Build the legal game cache

1. Clone/download this repository to a writable folder, or extract `HL2VR-Standalone-0.986.7z`. Keep `BUILD_GAME_CACHE.*`, `tools/` and `vr_game_resources/` beside one another.
...
```bash
adb devices
adb install -r HL2VR-Standalone-0.986.apk
```
